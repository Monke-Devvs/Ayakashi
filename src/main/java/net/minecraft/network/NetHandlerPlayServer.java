...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
            ...
            case STOP_DESTROY_BLOCK:
                ...
                if (d3 > 36.0D) {
                    ...
                    if (packetIn.getStatus() == C07PacketPlayerDigging.Action.START_DESTROY_BLOCK) {
                        if (!this.serverController.isBlockProtected() && worldserver.getWorldBorder().contains(blockpos)) {
    ...
    public void processPlayerBlockPlacement(C08PacketPlayerBlockPlacement packetIn) {
        ...
        if (packetIn.getPlacedBlockDirection() == 255) {
            ...
            if (this.hasMoved && this.playerEntity.getDistanceSq((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D) < 64.0D && !this.serverController.isBlockProtected() && worldserver.getWorldBorder().contains(blockpos)) {
